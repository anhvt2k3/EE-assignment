        ...
        <script>
            ...
            function submitForm(action, form) {
                if (action === 'carting') {
                    ...
                    form.action = "../controller/checkout.php?action=" + action;
                    form.submit();
                } else {
                    var cartitems = JSON.parse(localStorage.getItem("cartItems"));
                    var newInput = document.createElement("input");
                    newInput.setAttribute("type", "hidden");
                    newInput.setAttribute("name", "carts");
                    newInput.setAttribute("value", JSON.stringify(cartitems));
            ...
            while ($row = mysqli_fetch_assoc($result1)) {
                ?>
                <div class="col-sm-4">
            <form method="post" action="../controller/checkout.php" class="checkoutForm">
                <div class="card" style="width: 18rem; padding: 5px; margin: 5px;">
                    ...
                    <div class="card-body">
                        ...
                        <button type="button" class="btn btn-primary" onclick="submitForm('buying', this.form)" name="checkout"><span><span>Buy Now!</span></span></button>
                        <button type="button" class="btn btn-success" onclick="submitForm('carting', this.form)" name="addToCart"><span><span>Add to Cart</span></span></button>
                        <div style="margin-bottom: 10px;"></div>
                        <button type="button" href="../screens/checkout.php" class="btn btn-warning" onclick="submitForm('gotocart', this.form)" name="carts">Go to Carts!</button>
                    </div>
                </div>
            </form>
                </div>
